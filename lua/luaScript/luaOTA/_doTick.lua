...
return function() -- the proper doTick() timer callback
  ...
  if (tick_count < 50) then -- (wait up to 10 secs for Wifi connection)
  ...
  elseif (tick_count == 50) then -- assume timeout and exec app CB
    return self.startApp("OK: Timeout on waiting for wifi station setup")

  elseif (tick_count == 80) then -- wait up to 2.5 secs for TCP response
    gpio.write(0, gpio.HIGH)
    tmr.unregister(0)
    if ( conn ~= nil ) then
